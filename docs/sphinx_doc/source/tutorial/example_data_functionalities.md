...
service:
  data_juicer:
    ...
    port: 5005
data_processor:
  data_processor_url: 'http://127.0.0.1:5005'
...
Here you can set the information of the started data processor server, and some other items about reward shaping:
...
+ `experience_pipeline`: the configs for the experience pipeline. Experience pipeline is used to process the experiences outputted by the explorer, such as reward shaping, data filtering and augmentation. It consists of several inner configs:
  + `operators`: the operators used in the experience pipeline.
    ...
    + The actual configs for the "data_juicer" operator are stored in another file "examples/data_examples/new_exp_pipe/dj_scoring_exp.yaml", which will be introduced later.
  + `save_input`: whether to save the input experiences to the disk. Here we set it to false for simplicity. 

In default, the experiences are generated by the explorer and sent to experience piepline. After processed, they are outputted to the trainer.
