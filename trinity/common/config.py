...
class Config:
    ...
    mode: str = "both"  # `explore`, `train`, `both` or `test`
    ...
    def check_and_update(self) -> None:  # noqa: C901
        ...
        # check mode
        if self.mode not in ["explore", "train", "both", "test"]:
        ...
        if self.mode == "test" and self.synchronizer.sync_method != SyncMethod.CHECKPOINT:
            self.synchronizer.sync_method = "checkpoint"
            logger.warning(
                "Test mode only supports checkpoint synchronization, set `synchronizer.sync_method` to `checkpoint`."
            )
        if (
