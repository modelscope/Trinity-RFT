...
class TestRewardShapingMapper(unittest.TestCase):
    ...
    def test_experience_pipeline(self):
        ...
        repeat_times = 4
        step_num = 2
        experiences = get_experiences(task_num=task_num, repeat_times=repeat_times, step_num=step_num)
        ...
        for prev_exp, res_exp in zip(experiences, res_exps):
            self.assertEqual(
                (prev_exp.reward + prev_exp.info["quality_score"]) * 0.5 * prev_exp.info["difficulty_score"],
                res_exp.reward
            )
